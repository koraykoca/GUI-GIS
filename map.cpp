...
Map::Map(QWidget* parent, Qt::WindowFlags fl)
   : QMainWindow(parent,fl)
{
   ...
   //connect(this, SIGNAL(coordChanged(QgsPointXY)), this, SLOT(showCoord(QgsPointXY)));
...
void Map::showContextMenu(const QPoint &pos){
...
void Map::set_checks(QAction* action){
...
void Map::dropEvent(QDropEvent *event){
   ...
   Map::addLayer1(dataUrl.path(), name, ProviderName);
}
...
void Map::createLayer(QString type, QgsPointXY point){
...
//    map["size"]= "6";
   ...
   QgsRasterMarkerSymbolLayer* symbolLayer = new QgsRasterMarkerSymbolLayer(path);
   qDebug() << typeid(symbolLayer).name();
   symbol->appendSymbolLayer(symbolLayer);


   auto renderer = new QgsSingleSymbolRenderer(symbol);
...
void Map::showCoord(QgsPointXY point)
{
   ...
   emit coordChanged(point);
}

void Map::selectCoord(QgsPointXY point)
{
   if (layers.contains(ptrLayer4) == false){
...
void Map::mouseEvent(QgsPointXY point, Qt::MouseButton button){
...
void Map::addLayer1(QString myLayerPath, QString myLayerBaseName, QString myProviderName)
{
